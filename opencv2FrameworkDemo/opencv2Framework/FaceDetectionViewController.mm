...
//  FaceDetection
//
//  Created by wang piepie on 11-9-1.
...
- (void) opencvFaceDetect:(UIImage *)overlayImage  {
	...
	if(imageView.image) {
		...
		// Load XML
		NSString *path = [[NSBundle mainBundle] pathForResource:@"haarcascade_frontalface_default_old" ofType:@"xml"];
...
- (IBAction)loadImage:(id)sender 
{
	if(!actionSheetAction) 
    {
		UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@""
														...
														otherButtonTitles:@"Use Photo from Library", @"Take Photo with Camera", @"Use  m2u", nil];
...
- (IBAction)faceDetect:(id)sender 
{
	...
	if(imageView.image) {
		UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@""
																 delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil
														otherButtonTitles:@"Bounding Box", @"Handsome boy", nil];
		actionSheet.actionSheetStyle = UIActionSheetStyleDefault;
	...
	switch(actionSheetAction) {
		case ActionSheetToSelectTypeOfSource: {
			...
			if (buttonIndex == 0) {
				...
				NSString *path = [[NSBundle mainBundle] pathForResource:@"m2u" ofType:@"jpg"];
		...
		case ActionSheetToSelectTypeOfMarks: {
			...
			UIImage *image = nil;
			if(buttonIndex == 1) {
				NSString *path = [[NSBundle mainBundle] pathForResource:@"haha" ofType:@"png"];
